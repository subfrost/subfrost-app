...
export function YieldSnapshot() {
  ...
  const [dxBTCYield, setDxBTCYield] = useState(0)

  useEffect(() => {
    const interval = setInterval(() => {
      setFrostYield(prev => +(prev + Math.random() * 0.1).toFixed(2))
      setDxBTCYield(prev => +(prev + Math.random() * 0.05).toFixed(2))
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <Card className="bg-blue-900 text-white">
        <CardHeader>
          <CardTitle className="retro-text text-blue-300">dxBTC Yield</CardTitle>
        </CardHeader>
        <CardContent>
          <p className="text-2xl font-bold">{dxBTCYield.toFixed(2)}%</p>
          <p className="text-sm">Annual Percentage Yield</p>
        </CardContent>
      </Card>
    </div>
