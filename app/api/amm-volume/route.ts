...
function bucketByDay(
  ...
  // Floor at 2025-08-01: ensure the chart always covers from August 2025 onward,
  // even if the upstream RPC has no events that early. Inject a $0 floor point
  // so forward-fill produces continuous daily coverage from then to today. This
  // is what the user expects — "at minimum one value for each day since the
  // beginning of August 2025 through today".
  const FLOOR_DATE = '2025-08-01';
  if (entries.length === 0 || parseDay(entries[0]!.time) > parseDay(FLOOR_DATE)) {
    entries.unshift({ time: FLOOR_DATE, valueUsd: 0 });
  }
