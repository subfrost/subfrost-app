...
type Props = {
  txId: string;
  type: 'deposit' | 'withdraw' | 'claim' | 'harvest' | 'stake' | 'unstake';
  onClose: () => void;
};
...
const typeColors = {
  ...
  withdraw: 'from-purple-500 to-purple-600',
  claim: 'from-green-500 to-green-600',
  harvest: 'from-yellow-500 to-yellow-600',
  stake: 'from-indigo-500 to-indigo-600',
  unstake: 'from-pink-500 to-pink-600',
};

export default function VaultSuccessNotification({ txId, type, onClose }: Props) {
    ...
    <>
      ...
      >
        <div className="relative rounded-2xl border-2 border-green-500/30 bg-gradient-to-br from-green-50 to-white p-4 shadow-[0_12px_48px_rgba(34,197,94,0.25)] backdrop-blur-xl">
          {/* Success icon */}
          <div className={`absolute -top-3 -left-3 flex h-10 w-10 items-center justify-center rounded-full border-2 border-white bg-gradient-to-br ${typeColors[type]} shadow-[0_4px_16px_rgba(34,197,94,0.4)]`}>
          ...
          <div className="ml-8 pr-8">
            <div className="flex items-center gap-2 mb-1">
              <h3 className="text-base font-bold text-green-800">{typeLabels[type]}</h3>
