...
/**
 * Shows a notice overlay when a feature is gated.
 * - swap/futures: gated by DEMO_MODE_ENABLED + mainnet (useDemoGate), session-dismissible.
 * - vaults: always gated on mainnet (useVaultDemoGate), resets every navigation (no session key).
 */
export default function MainnetFeatureNotice({ children, feature }: MainnetFeatureNoticeProps) {
  const { t } = useTranslation();
  const isDemoGated = useDemoGate();
  const isVaultGated = useVaultDemoGate();
  const [dismissed, setDismissed] = useState(false);

  const gated = feature === 'vaults' ? isVaultGated : isDemoGated;

  if (!gated || dismissed) {
    return <>{children}</>;
  }

  const title = feature === 'vaults'
    ? t('featureNotice.vaultsTitle')
