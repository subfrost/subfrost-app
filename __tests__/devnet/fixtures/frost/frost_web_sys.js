 ...
 * @param {number} signers
 * @param {number} threshold
 * @returns {string}
 */
export function generate_frost_keys(signers, threshold) {
    let deferred2_0;
    let deferred2_1;
    try {
        const ret = wasm.generate_frost_keys(signers, threshold);
        var ptr1 = ret[0];
        var len1 = ret[1];
        if (ret[3]) {
            ptr1 = 0; len1 = 0;
 ...
 * @returns {Uint8Array}
 */
export function sign_sighash(bundle_json, sighash) {
    ...
    const ret = wasm.sign_sighash(ptr0, len0, ptr1, len1);
    if (ret[3]) {
        throw takeFromExternrefTable0(ret[2]);
    }
 ...
 * @returns {Uint8Array}
 */
export function get_group_public_key(bundle_json) {
    const ptr0 = passStringToWasm0(bundle_json, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('frost_web_sys_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
