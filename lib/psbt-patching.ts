...
export function patchInputsOnly(params: PatchInputsOnlyParams): {
  ...
  // gracefully no-op when their respective addresses are missing; the
  // failure mode used to be a cryptic `bitcoin.address.toOutputScript(undefined)`,
  // which we now avoid by accepting the absence and skipping P2TR patches.
