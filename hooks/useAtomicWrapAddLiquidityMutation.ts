 ...
 * for an atomic wrap+addLiquidity. Two chained protostones:
 *   p0: [32,0,FRBTC_WRAP_OPCODE]:p1:v1
 *       — wraps BTC, forwards (auto-allocated X + minted frBTC) to p1
 *   p1: factory.AddLiquidity (opcode 11) with full Uniswap-style params
 *
 ...
 *
 * Verified: frBTC forwards in lib.rs:496; factory opcode 11 signature in
 * fujin-factory/src/lib.rs:63.
 */
'use client';
...
export function useAtomicWrapAddLiquidityMutation() {
  ...
  const executeAtomicAddLiquidity = useCallback(
    async (params: AtomicWrapAddLiquidityParams) => {
      ...
      // Convert BTC → sats; apply wrap fee to compute frBTC actually arriving at addLiquidity
      ...
      // Slippage protection (amount_*_min)
    ...
    [network, address, premiumData, fee.feeRate, addLiquidityMutation],
  );
