...
## Metabot SSH-tunnel checklist

`regtest-local` runs on the `metabot` deployment host inside the team OpenVPN. The frontend's `regtest-local` config points at `localhost:18888/18889/50010`, which only resolves if an SSH port-forward is active.

**Bring up the tunnel** (SSH user is `ubuntu`):
```bash
ssh -fN \
...
NodePort map: alkanes-jsonrpc-np 31945 → svc/alkanes-jsonrpc:18888, esplora-np 31050 → 50010, metashrew-nodeport 31080 → 8080. The dev-server RPC proxy and the WASM provider both target our `localhost:18888` (and `:18889` for batch).
...
**Mining is manual.** metabot's bitcoind doesn't auto-mine. Mempool txs are flagged `unbroadcast=true` by default — `generatetoaddress` produces empty blocks that don't include them. After every state-mutating tx, mine via the wallet/settings page Mine 1 Block button or:
```bash
...
**`regtest-local` case-statement gotcha.** When adding new flows or touching network-conditional code, grep for `regtest-local` and confirm it's listed wherever the other regtest variants are. Common gaps:
...
- `useWrapMutation` / `useWrapSwapMutation` dynamic-signer trigger — gate is `network === 'devnet' || network === 'regtest-local'`. The metabot signer key may differ from the hosted one; use `getSignerAddressDynamic` to query opcode 103 on `[32:0]` and derive via `bitcoin.payments.p2tr({ internalPubkey })`.
- `app/api/rpc/[[...segments]]/route.ts` — devnet returns 503 to force the in-browser interceptor; `regtest-local` must NOT be conflated with devnet here.
...
Verified 2026-04-26 on metabot:
...
2. Mine via the wallet/settings page Mine 1 Block button, NOT `prioritisetransaction` — the latter combined with re-broadcast is what produced the zombie state.
